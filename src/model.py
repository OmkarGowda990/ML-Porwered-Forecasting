...
from src.settings import HOLIDAY_NAME_MAP, PROPHET_PARAMS
...
class ProphetModel:
    ...
    def fit(self, price_series: pd.Series) -> ProphetModel:
        ...
        if not holidays.empty:
            ...
            logging.info(f"Using {len(holidays)} trading holidays for Prophet model")
        else:
            logging.warning("No holidays found for date range, using Prophet without holidays")
